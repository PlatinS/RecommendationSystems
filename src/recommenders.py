...
class MainRecommender:
    ...
    def get_similar_items_recommendation(self, user, N=5, own=False):
        """Рекомендуем товары, похожие на топ-N купленных юзером товаров"""
        
        model = self.own_recommender if own else self.model
    ...
    def get_similar_users_recommendation(self, user, N=5):
        ...
        for user_id, _ in similar_users:
            own_recs = self.get_similar_items_recommendation(user_id, 1, own=True)
            res += own_recs
